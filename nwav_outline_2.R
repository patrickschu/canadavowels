...
canada = read.csv("~/Downloads/canadavowels-master/canada_final_1121.csv", header=T)
summary(canada)
original = subset(canada, LOCATION == "WA")
summary(original)
# 4821 tokens
...
### OVERALL
cc=ggvowels(can, "The Washington Vowel System by age group", "F1labovstandard", "F2labovstandard", "AgeGrp")
#F1 model
var=can[can$VOWEL == "TRAP", ]
linreg=lm(scale(F1labovstandard)~scale(AGE)+ETHNICITY, data=var)
summary(linreg)
...
#F2  model
var=can[can$VOWEL == "TRAP", ]
linreg=lm(scale(F2labovstandard)~scale(AGE)+ETHNICITY, data=var)
summary(linreg)
confint(linreg)
...
##BRINGING IN THE WASHINGTONIANS
