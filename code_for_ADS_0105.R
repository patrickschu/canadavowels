save.image()
setwd("E:/cygwin/home/ps22344/Downloads/canadavowels/")
library(vowels)

##########

##MAIN
original=read.csv("canada_final_1121.csv", header=T)
...
##PLOTTING##
gg=ggplot(data=dataset, aes(x=F2, y=F1));
gg=
gg+
theme_classic()+
...
geom_point(data=totalmeanies, aes(x=F2means, y=F1means, colour=SEPARATOR, label=VOWEL, group=VOWEL), size=6, alpha=0.7)+
...
cc=ggvowels(can2, "The Canadian Shift in Vancouver", "F1labovstandard", "F2labovstandard", "AgeGrp")
can1=can[can[["VOWEL"]] != "FLEECE" && can[["VOWEL"]] != "GOOSE", ] 
cc=ggvowels(can1, "The Canadian Shift", "F1labovstandard", "F2labovstandard", "AgeGrp")
cc=ggvowels(can, "The Canadian Shift US v CAN", "F1labovstandard", "F2labovstandard", "LOCATION")
